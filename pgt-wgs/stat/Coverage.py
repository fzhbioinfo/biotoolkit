...
def statistics(args):
    ...
    for row in bed.itertuples(index=False):
        ...
        dic['start'] = row.start
        dic['stop'] = row.stop
        dic['size'] = row.stop - row.start
        dic['reads_on_target(rm_dup)'] = bam.count(row.chromosome, row.start, row.stop, read_callback='all')
        dic['reads_on_target(dup)'] = bam.count(row.chromosome, row.start, row.stop, read_callback=lambda x: x.is_duplicate)
        cover = bam.count_coverage(row.chromosome, row.start, row.stop, read_callback='all')
        dic['base_on_target(rm_dup)'] = np.sum(cover)
        dic['base_on_target(dup)'] = np.sum(bam.count_coverage(row.chromosome, row.start, row.stop, read_callback=lambda x: x.is_duplicate))
...
def analysis(args):
    if args.bed:
        ...
        logger.info(f'Use split bed, bed_dir is: {args.bed_dir}')
        chromosomes = [f'chr{i}' for i in range(1, 23)] + ['chrX', 'chrY', 'chrM_NC_012920.1']
        bed_list = list()
