...
class ADO:
    ...
    def cal_ado(self, target, gene, dp_threshold, embryo_dp_threshold, use_indel=False):
        # 计算全基因组和基因区域的ADO,默认不用indel
        if gene == 'all_gene':
            reader = self.reader
        else:
            chromosome = target.loc[gene, 'chromosome']
            start_extend = target.loc[gene, 'start_extend']
            end_extend = target.loc[gene, 'end_extend']
            reader = self.reader.fetch(chromosome, start_extend, end_extend)
    ...
    def analysis(cls, args):
        ...
        target.columns = ['gene', 'chromosome', 'start_extend', 'end_extend', 'start', 'end']
        target.index = target['gene'].tolist()
        ado = cls(args.vcf)
        ado_stat = pd.DataFrame()
        for gene in ['all_gene'] + args.gene.split(','):
            stat_df = ado.cal_ado(target, gene, args.triosDepth, args.embryoDepth)
